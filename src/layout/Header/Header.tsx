...
import { toggleState, openModal, closeModal } from "../../app/modalSlice.ts";
...
export const Header: FC = () => {
  ...
  let modalState = useAppSelector((state) => state.modal.open)
    ...
    <header className="header">
      ...
      >
        <ul className="nav__list">
          <li className="nav__item" style={{ display: screenSize < 1000 ? "none" : "block" }}>
          ...
          <li className="nav__item">
            <Link className="nav__link" to="/genres">
              {screenSize < 1000 ? <GenresIcon className="nav__link--mobile" /> : 'Жанры'}
            </Link>
          </li>
        </ul>
        <InputField
          imgChild={<button
            children={searchQuery.isPending && inputValue !== '' ? <Loader /> : <SearchMobIcon />}
            title="search"
          ...
          child={
            <input
              ...
              placeholder='Поиск'
              type="text"
              value={searchQuery.isError ? searchQuery.error?.message : inputValue}
          ...
          btn={
            < Button
              className={inputValue !== '' ? "header__btn btn--close active" : "header__btn btn--close"}
              kind="round"
              children={<IconClose />}
      ...
      >
        < Button className="modal-close" kind="round" children={<IconClose />} onClick={onCloseModal} />
        {
          modalContext === 'dropdown' &&
          screenSize < 1000 &&
          <InputField
            imgChild={
              <div className="header__badge">
                {searchQuery.isPending && inputValue !== '' ? <Loader /> : <SearchMobIcon />}
              </div>
            }
            child={
              <input

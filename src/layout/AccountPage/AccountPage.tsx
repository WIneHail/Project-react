...
export const AccountPage: FC = () => {
  ...
  const [accountState, setAccountState] = useState<'favorites' | 'settings'>('favorites')

  const initials = user ? user?.name.charAt(0).toUpperCase() + user?.surname.charAt(0).toUpperCase() : 'Nan'
    ...
    <>
      ...
      <ul className="account__menu">
        <li className="account__item">
          <Button
            className={accountState === "favorites" ?
              "account__btn active" :
              "account__btn"}
            kind="linklike"
